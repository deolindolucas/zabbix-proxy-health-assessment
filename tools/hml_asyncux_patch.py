...
base = Path("/usr/share/zabbix/ui/modules/proxy_health_assessment")
...
"""
if old not in s:
    raise SystemExit("controller doAction block not found")
s = s.replace(old, new)
...
"""
if old not in s:
    raise SystemExit("js constructor block not found")
s = s.replace(old, new)
js.write_text(s)
